...
# ==========================================================================
# Strategy X: RSI + Supertrend + Relative Strength
# ===========================================================================

class RSISupertrendRelativeStrength(BaseStrategy):
    """
    Composite strategy combining RSI, Supertrend and a relative-strength
    comparison against a benchmark (e.g. Nifty index).

    ENTRY CONDITION (all must be true simultaneously):
      1. RSI(14) > 50
      2. Supertrend direction is bullish (st_direction == 1)
      3. Relative strength (stock vs benchmark over `rs_period` bars) > 0

    EXIT CONDITION: when **any** of the three entry conditions fail.  This is
    handled by emitting a sell signal whenever the combined long condition
    becomes false; the engine then closes the position on the next bar's open.

    SHORTS:
      Not allowed – the backtester will ignore any sell signals generated before
      a long entry because `allow_shorting` is normally set to False.  All
      signals produced by this strategy are therefore either 0 (no action),
      +1 (open/extend long) or -1 (close long).

    RELATIVE STRENGTH (RS) FORMULA:
        rs = (stock_close / stock_close.shift(rs_period))
             / (bench_close / bench_close.shift(rs_period)) - 1
    The input DataFrame **must** contain a column named ``bench_close`` which
    provides the benchmark's closing price indexed identically to the stock.
    A constant benchmark series (e.g. flat Nifty) produces RS = stock_gain - 1.

    PARAMETERS:
      rsi_period      : RSI lookback (default 14)
      super_period    : ATR period used by Supertrend (default 10)
      super_multiplier: ATR multiplier for Supertrend bands (default 3.0)
      rs_period       : Lookback length used for relative strength (default 55)

    INDICATOR COLUMNS ADDED:
      'rsi', 'supertrend', 'st_direction', 'rs'
    """
    ...
    def __init__(
        ...
        rsi_period:         int   = 14,
        super_period:       int   = 10,
        super_multiplier:   float = 3.0,
        rs_period:          int   = 55,
    ) -> None:
        super().__init__(
            name=(
                f"RSI+Supertrend+RS (rsi{rsi_period}, st{super_period},{super_multiplier}, "
                f"rs{rs_period})"
            ),
            description=(
                "Long when RSI>50, Supertrend bullish, and stock outperforms "
                "benchmark over the RS lookback. Exit when any condition fails."
            ),
        )
        self.rsi_period      = rsi_period
        self.super_period    = super_period
        self.super_multiplier= super_multiplier
        self.rs_period       = rs_period

    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Generate trading signals based on RSI, Supertrend, and Relative Strength.
        
        IMPORTANT: Data Alignment Requirement
        =====================================
        The DataFrame MUST have a 'bench_close' column containing benchmark (index) close prices.
        
        If stock and benchmark data have different date ranges:
          • Use INNER merge on dates — only calculate signals for overlapping dates
          • Example: If stock is 2007-01-02:2024-12-31 and benchmark is 2007-01-01:2024-12-31,
                    only dates from 2007-01-02 onward will be in the backtest
          • NEVER use direct assignment like df['bench_close'] = other_df['close'] 
                    (this misaligns dates by position)
        
        Proper merge pattern:
            merged = stock_df.merge(
                bench_df[['close']].rename(columns={'close': 'bench_close'}),
                left_index=True, right_index=True, how='inner'
            )
        
        This ensures each row's stock_close and bench_close are from the SAME date.
        """
        df = self._validate_and_prepare(df)
        ...
        if "bench_close" not in df.columns:
            raise ValueError(
                f"Strategy '{self.name}': DataFrame must include 'bench_close' "
                "column containing benchmark close prices."
            )
        ...
        # Supertrend
        st = supertrend(df, self.super_period, self.super_multiplier)
        df["supertrend_upper"] = st["supertrend_upper"]
        df["supertrend_lower"] = st["supertrend_lower"]
        df["supertrend_signal"] = st["supertrend_signal"]
        # Relative strength vs benchmark
        stock_gain = df["close"] / df["close"].shift(self.rs_period)
        bench_gain = df["bench_close"] / df["bench_close"].shift(self.rs_period)
        df["rs"] = stock_gain / bench_gain - 1

        # Long condition (all three must hold)
        long_cond = (
            (df["rsi"] > 50) &
        ...
        # sell when the long condition becomes false
        df.loc[~long_cond, "signal"] = -1

        warmup = max(self.rsi_period, self.super_period * 2, self.rs_period)
        return self._suppress_warmup_signals(df, warmup)
