...
cfg = BacktestConfig(
    ...
    segment             = Segment.EQUITY_DELIVERY,
    default_order_type  = OrderType.MARKET,
    limit_offset_pct    = 0,          # 0.2% below close for buy limit
    stop_loss_pct       = 0,          # 1.5% fixed stop on all trades
    use_trailing_stop   = False,
    trailing_stop_pct   = 0,          # 2% trailing stop
    capital_risk_pct    = 1,        # risk 1.5% of capital per trade
    max_positions       = 1,
    max_drawdown_pct    = 1,         # halt at 15% drawdown
    allow_shorting      = False,
    ...
    save_raw_data       = True,
    generate_summary    = True,
    run_label           = 'ema_cross_reliance',
    max_candles         = 2500       # show full trade duration on chart
)
